...
class Stage(models.IntegerChoices):
    ...
    STAGE_2 = 2, "Stage 2: Acceptance"
    STAGE_3 = 3, "Stage 3: Certificate"
    STAGE_4 = 4, "Stage 4: Opposition"
...
class SubStage(models.TextChoices):
    ...
    EXAMINATION_DONE = "examination_done", "EXAMINATION"
    ...
    # Stage 3
    DEMAND_NOTE_ISSUE = "demand_note_issue", "DEMAND NOTE ISSUE"
...
SUB_STAGE_TO_STAGE = {
    ...
    SubStage.EXAMINATION_DONE: Stage.STAGE_1,

    SubStage.ACCEPTANCE_DONE: Stage.STAGE_2,
    SubStage.HEARING: Stage.STAGE_2,
    SubStage.PUBLISHED: Stage.STAGE_2,

    SubStage.DEMAND_NOTE_ISSUE: Stage.STAGE_3,
    SubStage.DEMAND_NOTE_RECEIVED: Stage.STAGE_3,
    SubStage.DEMAND_NOTE_SUBMITTED: Stage.STAGE_3,
    SubStage.CERTIFICATE_ISSUED: Stage.STAGE_3,
    SubStage.CERTIFICATE_RECEIVED: Stage.STAGE_3,
    SubStage.CERTIFICATE_PRINT: Stage.STAGE_3,
    SubStage.CERTIFICATE_DISPATCH: Stage.STAGE_3,

    SubStage.OPPOSITION_RECEIVED: Stage.STAGE_4,
    SubStage.OPPOSITION_WITHDRAWN: Stage.STAGE_4,
    SubStage.OPPOSITION_FILED: Stage.STAGE_4,
}
...
# DDMM = day+month of creation date
# NNN = 3-digit sequential number (global)

def _generate_ref_number():
    """Generate unique internal reference number: XX-DDMM-NNN"""
    from datetime import date
    today = date.today()
    ddmm = today.strftime("%d%m")
    letters = ''.join(random.choices(string.ascii_uppercase, k=2))
    # Find next sequence across all existing ref numbers with same DDMM
    existing = Application.objects.filter(
        ref_number__contains=f"-{ddmm}-"
    ).order_by('-ref_number')
    if existing.exists():
        last = existing.first().ref_number  # e.g. BX-2904-037
        parts = last.split('-')
        try:
        ...
        next_seq = 1
    candidate = f"{letters}-{ddmm}-{next_seq:03d}"
    # Ensure uniqueness
    while Application.objects.filter(ref_number=candidate).exists():
        letters = ''.join(random.choices(string.ascii_uppercase, k=2))
        candidate = f"{letters}-{ddmm}-{next_seq:03d}"
    return candidate
...
class Application(models.Model):
    # ── Internal Reference (auto-generated, immutable) ──
    ...
    # ── Case / Folder number (client-type based) ──
    ...
    city = models.CharField(max_length=50, blank=True, choices=[
        ("ISB", "Islamabad"), ("LHR", "Lahore"), ("KHI", "Karachi"), ("PES", "Peshawar")
    ])
    ...
    # ── Stage Control ──
    proceed_to_stage2 = models.BooleanField(default=False)
    proceed_datetime = models.DateTimeField(null=True, blank=True)
    stop_status = models.CharField(max_length=20, choices=StopStatus.choices, blank=True)

    # ── Journal (Stage 2 Publication) ──
    journal_number = models.PositiveIntegerField(null=True, blank=True)
    journal_date = models.DateField(null=True, blank=True)
    journal_screenshot = models.ImageField(upload_to="journals/%Y/%m/", null=True, blank=True)
    ...
    def generate_case_number(self):
        """Generate case number: [ClientType]-[ClientID]-[Sequence]"""
        if not self.client_id:
            raise ValueError("client_id required for auto-generation")
    ...
    def get_stage3_deadline(self):
        """Stage 3 deadline: filing + 135 days"""
    ...
    def get_stage_color(self):
        colors = {1: 'stage1', 2: 'stage2', 3: 'stage3', 4: 'stage4'}
        return colors.get(self.current_stage, 'stage1')
    ...
    def save(self, *args, **kwargs):
        ...
        # Auto-generate ref_number on creation
        if not self.ref_number:
            self.ref_number = _generate_ref_number()
        ...
        if is_create:
            AuditLog.log(
                ...
                old_value="",
                new_value=f"created case_number={self.case_number} ref={self.ref_number}",
            )
            return
        ...
        tracked_fields = [
            "case_number", "client_type", "client_id", "sequence",
            "application_name", "application_type", "trademark_no",
            "class_numbers", "application_year", "filing_date", "case_no",
            "applicant_name", "trading_as", "applicant_type", "address",
            "city", "agent_name", "agent_address", "jurisdiction",
            "dispatch_status", "demand_note_date", "proceed_to_stage2",
            "proceed_datetime", "stop_status", "journal_number",
            "journal_date", "journal_screenshot", "logo",
            "current_stage", "current_sub_stage", "current_status",
        ]
        ...
        for field in tracked_fields:
            ...
            if old_value != new_value:
                action = AuditActionType.STATUS_CHANGED if field in {
                    "current_stage", "current_sub_stage", "current_status"
                } else AuditActionType.UPDATED
                AuditLog.log(
                    application=self,
                    action_type=action,
                    field_changed=field,
...
class Event(models.Model):
    ...
    def __str__(self):
        return f"{self.application.case_number}: {self.get_event_type_display()}"
    ...
    def save(self, *args, **kwargs):
        ...
        if is_create:
            ...
            AuditLog.log(
                ...
                field_changed="event",
                old_value="",
                new_value=f"{self.event_type} @ {self.event_datetime.isoformat()} sub={sub_stage_value}",
            )
...
class DocumentLink(models.Model):
    ...
    def __str__(self):
        return f"{self.application.case_number} document"
...
class AuditLog(models.Model):
    """Append-only audit trail — no delete allowed"""
    ...
    def __str__(self):
        return f"{self.application.case_number} {self.action_type} {self.field_changed}"

    def delete(self, *args, **kwargs):
        # Prevent deletion of audit logs — immutable record
        raise Exception("AuditLog records cannot be deleted — immutable audit trail.")

    @classmethod
    def log(cls, *, application, action_type, field_changed, old_value, new_value, changed_by=None):
...
class Agent(models.Model):
    ...
    address = models.TextField(blank=True)
    city = models.CharField(max_length=10, blank=True, choices=[
        ("ISB", "Islamabad"), ("LHR", "Lahore"), ("KHI", "Karachi"), ("PES", "Peshawar")
    ])
...
class FileUpload(models.Model):
    ...
    uploaded_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL,
        null=True, blank=True, related_name="file_uploads",
    )
    ...
    def __str__(self):
        return f"{self.application.case_number} - {self.file.name}"
